...
export const PortalChoiceModal: React.FC = () => {
  const { isPortalChoiceOpen, setIsPortalChoiceOpen, setWorkspaceMode, openAuthModal, currentUser, setActiveTab } = useApp();
  ...
  const enter = (mode: 'student'|'organization', tab: 'login'|'register') => {
    ...
    if (currentUser) {
      setActiveTab(mode === 'organization' ? 'organization' : 'dashboard');
    } else {
      openAuthModal(tab);
    }
  };
  ...
  return <AnimatePresence><motion.div className="fixed inset-0 z-[80] bg-slate-950/65 backdrop-blur-md flex items-center justify-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
    <motion.div initial={{opacity:0,y:28,scale:.96}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:20,scale:.97}} className="w-full max-w-4xl overflow-hidden rounded-[32px] border border-white/15 bg-[#100f18] text-white shadow-2xl">
      <div className="p-6 sm:p-8 border-b border-white/10 flex items-start justify-between gap-4 bg-gradient-to-r from-violet-950/80 via-slate-950 to-emerald-950/60">
        <div><p className="text-[11px] font-black tracking-[.22em] uppercase text-emerald-300">Choose your workspace</p><h2 className="mt-2 text-2xl sm:text-3xl font-black font-['Outfit',sans-serif]">Student Opportunity Engine</h2><p className="mt-2 max-w-2xl text-sm text-slate-300">One account can access both experiences. Choose where you want to work now.</p></div>
      ...
      <div className="px-7 pb-6 text-xs text-slate-400 flex items-center gap-2">You can switch workspace later without creating another account. <ArrowRight className="w-3.5 h-3.5"/></div>
    </motion.div>
