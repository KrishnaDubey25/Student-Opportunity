...
export const OpportunityCard: React.FC<OpportunityCardProps> = ({ opportunity, featured = false }) => {
  ...
  const handleAttachment = (file?: File) => {
    setAttachmentError('');
    if (!file) return;
    if (!file.type.startsWith('image/')) { setAttachmentError('Upload an image file for application proof.'); return; }
    if (file.size > 2 * 1024 * 1024) { setAttachmentError('Keep the image under 2 MB.'); return; }
      ...
      {showApply && (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[95] bg-slate-950/65 backdrop-blur-sm flex items-center justify-center p-4">
          <motion.div initial={{opacity:0,y:20,scale:.97}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:12,scale:.98}} className="w-full max-w-2xl rounded-[30px] bg-white p-6 sm:p-7 shadow-2xl max-h-[92vh] overflow-y-auto">
            ...
            <div className="application-image-panel mt-4 rounded-2xl border border-[#dfd2c6] bg-[#fbf6f0] p-4">
              <div className="flex items-start justify-between gap-3">
                <div><div className="text-sm font-extrabold text-[#2c221c]">Application image / proof <span className="text-[#8a776a] font-semibold">(optional)</span></div><div className="mt-1 text-[12px] font-semibold text-[#73645a]">Upload a screenshot, poster, certificate or proof image. Your college reviewer can view the exact file.</div></div>
                <Paperclip className="w-4 h-4 text-[#795640]"/>
              </div>
              <label className="application-image-picker mt-3 flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-[#c9b5a4] bg-white px-3 py-3 text-[12px] font-extrabold text-[#604536] hover:bg-[#f3e8de] transition">
                <ImageIcon className="w-4 h-4"/> {attachment ? attachment.name : 'Choose image'}
                <input type="file" accept="image/*" className="hidden" onChange={e=>handleAttachment(e.target.files?.[0])}/>
              </label>
              {attachment && <img src={attachment.dataUrl} alt="Application attachment preview" className="application-image-preview mt-3 h-40 w-full rounded-xl border border-[#dfd2c6] object-contain p-1"/>}
              {attachmentError && <div className="mt-2 text-[11px] font-bold text-rose-600">{attachmentError}</div>}
            </div>
            <div className="mt-6 flex justify-end gap-2"><button onClick={()=>setShowApply(false)} className="px-4 py-2.5 rounded-xl border border-slate-200 text-xs font-black">Cancel</button><button onClick={()=>{ if(isHackathon){ if(participation==='Team' && !teamName.trim()) return; submitOpportunityApplication(opportunity.id,participation,teamName,members,attachment || undefined); } else { submitOpportunityApplication(opportunity.id,'Individual',undefined,undefined,attachment || undefined); } setShowApply(false); }} className="v13-primary-button px-4 py-2.5 rounded-xl bg-[#463126] text-white text-xs font-black">Submit Application</button></div>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
