  ...
  return <div className="space-y-7 pb-20">
    <section className="org-premium-hero relative overflow-hidden rounded-[32px] bg-gradient-to-br from-white via-emerald-50/70 to-amber-50/60 text-slate-950 p-7 sm:p-9 border border-emerald-100 shadow-sm">
      ...
      <div className="relative z-10 flex flex-col xl:flex-row xl:items-end justify-between gap-7">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white px-3 py-1.5 text-[11px] uppercase tracking-[.24em] font-black text-emerald-700"><Radio className="w-3.5 h-3.5"/>Live Organization OS</div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">{(selectedCollege.campusFocus || []).map(item => <span key={item} className="rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-black text-emerald-700">{item}</span>)}</div>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => setShowMessage(true)} className="px-5 py-3.5 rounded-2xl bg-emerald-600 text-white text-sm font-black flex items-center gap-2 hover:-translate-y-0.5 transition"><Send className="w-4 h-4" />Broadcast Update</button>
        </div>
      </div>
    </section>
    ...
    <section className="rounded-[32px] border border-slate-200 bg-white shadow-sm overflow-hidden">
      <div className="p-6 sm:p-7 border-b border-slate-100 flex flex-col lg:flex-row gap-4 lg:items-center justify-between">
        <div><div className="text-[11px] font-black uppercase tracking-[.18em] text-violet-700">Campus Execution Layer</div><h2 className="mt-1 text-3xl font-black text-slate-950 font-['Outfit',sans-serif]">Tasks, study channels & collaborations</h2><p className="mt-1 text-sm text-slate-500">Anything you publish here appears on the student side and stays synced across open tabs.</p></div>
      ...
      {opsTab==='tasks' && <div className="grid xl:grid-cols-[.85fr_1.15fr] gap-0">
        <form onSubmit={e=>{e.preventDefault(); if(!taskForm.title.trim()) return; campusOps.addTask({title:taskForm.title,description:taskForm.description,category:taskForm.category as any,dueDate:taskForm.dueDate,assignedTo:taskForm.assignedTo || undefined,createdBy:selectedCollege.shortName}); setTaskForm({...taskForm,title:'',description:''});}} className="p-6 border-b xl:border-b-0 xl:border-r border-slate-100 bg-slate-50/60">
          <div className="text-sm font-black text-slate-950 flex items-center gap-2"><CheckSquare2 className="w-4 h-4 text-violet-700"/>Create a campus task</div>
      ...
      {opsTab==='resources' && <div className="grid xl:grid-cols-[.85fr_1.15fr]">
        <form onSubmit={e=>{e.preventDefault(); if(!resourceForm.title.trim()||!resourceForm.url.trim()) return; campusOps.addResource({title:resourceForm.title,subject:resourceForm.subject,description:resourceForm.description,level:resourceForm.level,links:[{label:resourceForm.label,url:resourceForm.url}]}); setResourceForm({...resourceForm,title:'',description:'',url:'https://'});}} className="p-6 border-b xl:border-b-0 xl:border-r border-slate-100 bg-emerald-50/40"><div className="text-sm font-black flex items-center gap-2"><BookOpenCheck className="w-4 h-4 text-emerald-700"/>Publish a study resource channel</div><div className="mt-4 grid gap-3"><input className="field" placeholder="Channel title" value={resourceForm.title} onChange={e=>setResourceForm({...resourceForm,title:e.target.value})}/><input className="field" placeholder="Subject / topic" value={resourceForm.subject} onChange={e=>setResourceForm({...resourceForm,subject:e.target.value})}/><textarea className="field min-h-20" placeholder="What should students study?" value={resourceForm.description} onChange={e=>setResourceForm({...resourceForm,description:e.target.value})}/><div className="grid sm:grid-cols-2 gap-3"><input className="field" placeholder="Link label" value={resourceForm.label} onChange={e=>setResourceForm({...resourceForm,label:e.target.value})}/><input className="field" placeholder="https://..." value={resourceForm.url} onChange={e=>setResourceForm({...resourceForm,url:e.target.value})}/></div><button className="rounded-2xl bg-emerald-700 text-white px-4 py-3 text-sm font-black">Publish Resource Channel</button></div></form>
        <div className="p-6 space-y-3">{campusOps.resources.map(r=><div key={r.id} className="rounded-2xl border border-slate-200 p-4 flex items-start justify-between gap-3"><div><div className="text-sm font-black">{r.title}</div><div className="mt-1 text-[11px] font-bold text-emerald-700">{r.subject} • {r.level}</div><p className="mt-2 text-xs text-slate-600">{r.description}</p><div className="mt-3 flex gap-2">{r.links.map(l=><a key={l.url} href={l.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[11px] font-black text-violet-700"><Link2 className="w-3 h-3"/>{l.label}</a>)}</div></div><button onClick={()=>campusOps.removeResource(r.id)} className="p-2 rounded-xl hover:bg-red-50 text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4"/></button></div>)}{!campusOps.resources.length && <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-slate-500">No study channel published yet.</div>}</div>
      </div>}
    ...
    <section className="grid xl:grid-cols-[1.25fr_.75fr] gap-5">
      <div className="rounded-[30px] border border-slate-200 bg-white overflow-hidden shadow-sm">
        <div className="p-6 border-b border-slate-100 flex flex-col lg:flex-row gap-4 lg:items-center justify-between"><div><div className="text-[11px] font-black uppercase tracking-[.18em] text-violet-700">Student Population</div><h2 className="mt-1 font-black text-slate-950 text-2xl">Student Directory & Progress</h2><p className="text-sm text-slate-500 mt-1">Open a student to inspect profile, skills, projects, certifications and application journey.</p></div><div className="flex gap-2 flex-wrap"><div className="relative"><Search className="w-4 h-4 absolute left-3 top-3 text-slate-400"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search student, roll, class..." className="pl-9 pr-3 py-2.5 rounded-xl border border-slate-200 text-xs w-full sm:w-64 outline-none focus:border-violet-400"/></div><div className="relative"><Filter className="w-3.5 h-3.5 absolute left-3 top-3 text-slate-400"/><select value={yearFilter} onChange={e=>setYearFilter(e.target.value)} className="pl-8 pr-8 py-2.5 rounded-xl border border-slate-200 text-xs font-bold bg-white"><option>All</option>{years.map(y=><option key={y}>{y}</option>)}</select></div></div></div>
        <div className="divide-y divide-slate-100">{students.map(u=>{const progress=profileProgress(u); const appCount=campusApps.filter(a=>a.studentUserId===u.id).length; return <div key={u.id} className="p-5 grid lg:grid-cols-[1.3fr_.75fr_1fr_.75fr_auto] gap-4 items-center hover:bg-violet-50/35 transition-colors"><div><div className="font-black text-slate-950">{u.name}</div><div className="text-xs text-slate-500 mt-1">{u.studentId || u.profile.studentId} • {u.profile.degree} • {u.profile.year}</div></div><div><div className="text-[10px] font-black uppercase text-slate-400">Profile</div><div className="mt-1 text-lg font-black">{progress}%</div></div><div><div className="text-[10px] font-black uppercase text-slate-400">Goal</div><div className="mt-1 text-xs font-bold text-slate-700 line-clamp-2">{u.profile.careerGoal || 'Not set'}</div></div><div><div className="text-[10px] font-black uppercase text-slate-400">Applications</div><div className="mt-1 text-lg font-black">{appCount}</div></div><button onClick={()=>setProfileStudent(u)} className="rounded-xl bg-slate-950 text-white px-3 py-2 text-[11px] font-black inline-flex items-center gap-1"><Eye className="w-3.5 h-3.5"/>View Profile</button></div>})}{!students.length && <div className="p-10 text-center text-sm text-slate-500">No matching students yet.</div>}</div>
      </div>

      <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div><div className="text-[11px] font-black uppercase tracking-[.18em] text-emerald-700">Live Applications</div><h2 className="mt-1 text-2xl font-black">Who applied, when & where</h2></div><Activity className="w-5 h-5 text-emerald-700"/></div><div className="mt-5 space-y-3 max-h-[560px] overflow-y-auto">{campusApps.slice().sort((a,b)=>new Date(b.appliedAt).getTime()-new Date(a.appliedAt).getTime()).map(a=><button key={a.id} onClick={()=>setApplicationDetail(a)} className="w-full rounded-2xl border border-slate-200 p-4 text-left hover:border-emerald-300 hover:-translate-y-0.5 transition"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-black">{a.studentName}</div><div className="mt-1 text-xs text-slate-500">{a.opportunityTitle}</div></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black">{a.status}</span></div><div className="mt-3 flex items-center gap-2 text-[10px] font-bold text-slate-400"><Clock3 className="w-3 h-3"/>{new Date(a.appliedAt).toLocaleString()} • {a.participationType}</div></button>)}{!campusApps.length && <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-slate-500">Applications will appear here live.</div>}</div></div>
    </section>

    <section className="grid md:grid-cols-3 gap-4">
      {[['Engagement Risk', `${atRiskStudents} students`, AlertTriangle, 'Profiles under 45% completion need intervention.'],['Placement Momentum', `${shortlistRate}% shortlist rate`, BrainCircuit, 'Tracks conversion from application to shortlist/selection.'],['Cross-Campus Network', `${campusOps.collaborations.length} collaborations`, UsersRound, 'Shows active partnerships and shared opportunities.']].map(([title,value,Icon,desc]:any)=><div key={title} className="rounded-[26px] border border-slate-200 bg-white p-5"><Icon className="w-5 h-5 text-violet-700"/><div className="mt-4 text-sm font-black text-slate-600">{title}</div><div className="mt-1 text-2xl font-black text-slate-950">{value}</div><p className="mt-2 text-xs text-slate-500">{desc}</p></div>)}
    </section>

    {profileStudent && <div className="fixed inset-0 z-[95] bg-slate-950/65 backdrop-blur-sm flex items-center justify-center p-4"><div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-[30px] bg-white p-6 shadow-2xl"><div className="flex items-start justify-between"><div><div className="text-[11px] font-black uppercase tracking-[.16em] text-violet-700">Student 360° Profile</div><h3 className="text-3xl font-black mt-1">{profileStudent.name}</h3><p className="text-sm text-slate-500 mt-1">{profileStudent.profile.degree} • {profileStudent.profile.year} • {profileStudent.studentId || profileStudent.profile.studentId}</p></div><button onClick={()=>setProfileStudent(null)} className="p-2 rounded-xl hover:bg-slate-100"><X className="w-5 h-5"/></button></div><div className="mt-6 grid sm:grid-cols-4 gap-3">{[['Profile',`${profileProgress(profileStudent)}%`],['GPA',profileStudent.profile.gpa||'—'],['Projects',profileStudent.profile.projects.length],['Applications',selectedProfileApps.length]].map(([k,v])=><div key={k} className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase text-slate-400">{k}</div><div className="mt-1 text-xl font-black">{v}</div></div>)}</div><div className="mt-6 grid md:grid-cols-2 gap-4"><div className="rounded-2xl border p-4"><div className="text-sm font-black">Skills</div><div className="mt-3 flex flex-wrap gap-2">{profileStudent.profile.skills.map(s=><span key={s.name} className="rounded-full bg-violet-50 px-2.5 py-1 text-[11px] font-bold text-violet-700">{s.name} • {s.level}</span>)}</div></div><div className="rounded-2xl border p-4"><div className="text-sm font-black">Career Goal</div><p className="mt-2 text-sm text-slate-600">{profileStudent.profile.careerGoal || 'Not set'}</p></div></div><div className="mt-4 rounded-2xl border p-4"><div className="text-sm font-black">Application Journey</div><div className="mt-3 space-y-2">{selectedProfileApps.map(a=><div key={a.id} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 p-3"><div><div className="text-xs font-black">{a.opportunityTitle}</div><div className="text-[10px] text-slate-400 mt-1">{new Date(a.appliedAt).toLocaleString()} • {a.participationType}{a.teamName?` • ${a.teamName}`:''}</div></div><span className="text-[10px] font-black">{a.status}</span></div>)}{!selectedProfileApps.length && <div className="text-xs text-slate-400">No application yet.</div>}</div></div></div></div>}

    {applicationDetail && <div className="fixed inset-0 z-[96] bg-slate-950/65 backdrop-blur-sm flex items-center justify-center p-4"><div className="w-full max-w-xl rounded-[30px] bg-white p-6 shadow-2xl"><div className="flex justify-between"><div><div className="text-[11px] font-black uppercase tracking-[.16em] text-emerald-700">Application Detail</div><h3 className="text-2xl font-black mt-1">{applicationDetail.opportunityTitle}</h3></div><button onClick={()=>setApplicationDetail(null)}><X className="w-5 h-5"/></button></div><div className="mt-5 grid sm:grid-cols-2 gap-3 text-sm"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] uppercase font-black text-slate-400">Student</div><div className="mt-1 font-black">{applicationDetail.studentName}</div><div className="text-xs text-slate-500">{applicationDetail.studentRoll}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] uppercase font-black text-slate-400">Applied At</div><div className="mt-1 font-black">{new Date(applicationDetail.appliedAt).toLocaleString()}</div></div></div>{applicationDetail.participationType==='Team' && <div className="mt-3 rounded-2xl border p-4"><div className="text-sm font-black">Team: {applicationDetail.teamName || 'Unnamed Team'}</div><div className="mt-2 flex flex-wrap gap-2">{applicationDetail.teamMembers.map(m=><span key={m} className="rounded-full bg-violet-50 px-2.5 py-1 text-[11px] font-bold text-violet-700">{m}</span>)}</div></div>}<div className="mt-5"><div className="text-xs font-black text-slate-500 mb-2">Review Status</div><div className="flex flex-wrap gap-2">{(['Applied','Under Review','Shortlisted','Selected','Rejected'] as const).map(status=><button key={status} onClick={()=>{updateApplicationReviewStatus(applicationDetail.id,status);setApplicationDetail({...applicationDetail,status});}} className={`px-3 py-2 rounded-xl text-[11px] font-black border ${applicationDetail.status===status?'bg-slate-950 text-white border-slate-950':'bg-white text-slate-600 border-slate-200'}`}>{status}</button>)}</div></div></div></div>}
